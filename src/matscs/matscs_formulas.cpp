...
void calculate_tcoeff_matrix(
    ...
    const MatrixXs v2_v3_v0 = v2 * v3_v0;
    ...
    const MatrixXs v1_v1_v1_v0 = v1_v1 * v1_v0;
    ...
    const MatrixXs v1_v0_v2_v0 = v1 * v0_v2_v0;
    ...
    const MatrixXs v1_v4_v0 = v1 * v4_v0;
    ...
    tDelta(0, 1).block(n, 0, n, n) = h1 * (-0.5*v1) + h2 * (0.5*v2) + h3 * (-0.5*v3) + h4 * (0.5*v4) + h5 * (-0.5*v5) + h6 * (0.5*v6);
    ...
    tDelta(0, 2).block(n, 0, n, n) = (0.5*v1) + h1 * (-1.5*v2) + h2 * (3.0*v3) + h3 * (-5.0*v4) + h4 * (7.5*v5) + h5 * (-10.5*v6);
    ...
    tDelta(0, 3).block(n, 0, n, n) = (1.0*v2) + h1 * (-5.0*v3) + h2 * (15.0*v4) + h3 * (-35.0*v5) + h4 * (70.0*v6);
    ...
    tDelta(0, 4).block(n, 0, n, n) = (2.5*v3) + h1 * (-17.5*v4) + h2 * (70.0*v5) + h3 * (-210.0*v6);
    ...
    tDelta(0, 5).block(n, 0, n, n) = (7.0*v4) + h1 * (-63.0*v5) + h2 * (315.0*v6);
    ...
    tDelta(0, 6).block(n, 0, n, n) = (21.0*v5) + h1 * (-231.0*v6);
    ...
    tDelta(0, 7).block(n, 0, n, n) = 66.0*v6;
    tDelta(0, 7).block(n, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(0, 8) = MatrixXs::Zero(2*n, 2*n);
    tH[0].resize(2*n, 2*n);
    tH[0].block(0, 0, n, n) = MatrixXs::Identity(n, n);    tH[0].block(0, n, n, n) = MatrixXs::Zero(n,n);     tH[0].block(n, 0, n, n) = MatrixXs::Zero(n,n);     tH[0].block(n, n, n, n) = MatrixXs::Identity(n, n);    tDelta(1, 0) = MatrixXs::Zero(2*n, 2*n);
    tDelta(1, 1).resize(2*n, 2*n);
    tDelta(1, 1).block(0, 0, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 1).block(0, n, n, n) = MatrixXs::Identity(n, n);
    tDelta(1, 1).block(n, 0, n, n) = h1 * (-0.5*v1) + h2 * (0.5*v2) + h3 * (-0.5*v3) + h4 * (0.5*v4) + h5 * (-0.5*v5) + h6 * (0.5*v6);
    tDelta(1, 1).block(n, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 2).resize(2*n, 2*n);
    tDelta(1, 2).block(0, 0, n, n) = h1 * (-0.5*v1) + h2 * (0.5*v2) + h3 * (-0.5*v3) + h4 * (0.5*v4) + h5 * (-0.5*v5) + h6 * (0.5*v6);
    tDelta(1, 2).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 2).block(n, 0, n, n) = (0.5*v1) + h1 * (-1.5*v2) + h2 * (3.0*v3) + h3 * (-5.0*v4) + h4 * (7.5*v5) + h5 * (-10.5*v6);
    tDelta(1, 2).block(n, n, n, n) = h1 * (-0.5*v1) + h2 * (0.5*v2) + h3 * (-0.5*v3) + h4 * (0.5*v4) + h5 * (-0.5*v5) + h6 * (0.5*v6);
    tDelta(1, 3).resize(2*n, 2*n);
    tDelta(1, 3).block(0, 0, n, n) = (0.5*v1) + h1 * (-1.5*v2) + h2 * (3.0*v3) + h3 * (-5.0*v4) + h4 * (7.5*v5) + h5 * (-10.5*v6);
    tDelta(1, 3).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 3).block(n, 0, n, n) = (1.5*v2) + h1 * (-0.125*v0_v1+-7.5*v3+0.125*v1_v0) + h2 * (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+22.5*v4) + h3 * (0.125*v3_v0+-0.125*v1_v2+-0.125*v0_v3+-0.125*v2_v1+-52.5*v5) + h4 * (0.125*v2_v2+0.125*v3_v1+105.0*v6+-0.125*v4_v0+0.125*v1_v3+0.125*v0_v4) + h5 * (-0.125*v2_v3+-0.125*v1_v4+-0.125*v4_v1+-0.125*v0_v5+0.125*v5_v0+-0.125*v3_v2);
    tDelta(1, 3).block(n, n, n, n) = (0.5*v1) + h1 * (-1.5*v2) + h2 * (3.0*v3) + h3 * (-5.0*v4) + h4 * (7.5*v5) + h5 * (-10.5*v6);
    tDelta(1, 4).resize(2*n, 2*n);
    tDelta(1, 4).block(0, 0, n, n) = (1.0*v2) + h1 * (-5.0*v3) + h2 * (15.0*v4) + h3 * (-35.0*v5) + h4 * (70.0*v6);
    tDelta(1, 4).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 4).block(n, 0, n, n) = (0.0833333333333*v0_v1+5.0*v3+-0.0833333333333*v1_v0) + h1 * (-0.25*v0_v2+-0.25*v1_v1+0.25*v2_v0+-35.0*v4) + h2 * (-0.5*v3_v0+0.416666666667*v1_v2+0.5*v0_v3+0.583333333333*v2_v1+140.0*v5) + h3 * (-0.75*v2_v2+-1.08333333333*v3_v1+-420.0*v6+0.833333333333*v4_v0+-0.666666666667*v1_v3+-0.833333333333*v0_v4) + h4 * (1.0*v2_v3+1.0*v1_v4+1.75*v4_v1+1.25*v0_v5+-1.25*v5_v0+1.25*v3_v2);
    tDelta(1, 4).block(n, n, n, n) = (1.0*v2) + h1 * (-5.0*v3) + h2 * (15.0*v4) + h3 * (-35.0*v5) + h4 * (70.0*v6);
    tDelta(1, 5).resize(2*n, 2*n);
    tDelta(1, 5).block(0, 0, n, n) = (2.5*v3) + h1 * (-17.5*v4) + h2 * (70.0*v5) + h3 * (-210.0*v6);
    tDelta(1, 5).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 5).block(n, 0, n, n) = (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+17.5*v4) + h1 * (0.625*v3_v0+-0.5*v1_v2+-0.625*v0_v3+-0.75*v2_v1+-157.5*v5) + h2 * (1.625*v2_v2+2.625*v3_v1+787.5*v6+-1.875*v4_v0+1.375*v1_v3+1.875*v0_v4) + h3 * (-3.25*v2_v3+-3.125*v1_v4+-6.875*v4_v1+-4.375*v0_v5+4.375*v5_v0+-4.25*v3_v2);
    tDelta(1, 5).block(n, n, n, n) = (2.5*v3) + h1 * (-17.5*v4) + h2 * (70.0*v5) + h3 * (-210.0*v6);
    tDelta(1, 6).resize(2*n, 2*n);
    tDelta(1, 6).block(0, 0, n, n) = (7.0*v4) + h1 * (-63.0*v5) + h2 * (315.0*v6);
    tDelta(1, 6).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 6).block(n, 0, n, n) = (-0.25*v3_v0+0.2*v1_v2+0.25*v0_v3+0.3*v2_v1+63.0*v5) + h1 * (-1.5*v2_v2+-2.5*v3_v1+-693.0*v6+1.75*v4_v0+-1.25*v1_v3+-1.75*v0_v4) + h2 * (5.05*v2_v3+4.75*v1_v4+11.5*v4_v1+7.0*v0_v5+-7.0*v5_v0+6.7*v3_v2);
    tDelta(1, 6).block(n, n, n, n) = (7.0*v4) + h1 * (-63.0*v5) + h2 * (315.0*v6);
    tDelta(1, 7).resize(2*n, 2*n);
    tDelta(1, 7).block(0, 0, n, n) = (21.0*v5) + h1 * (-231.0*v6);
    tDelta(1, 7).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 7).block(n, 0, n, n) = (0.5*v2_v2+0.833333333333*v3_v1+231.0*v6+-0.583333333333*v4_v0+0.416666666667*v1_v3+0.583333333333*v0_v4) + h1 * (-3.75*v2_v3+-3.5*v1_v4+-8.75*v4_v1+-5.25*v0_v5+5.25*v5_v0+-5.0*v3_v2);
    tDelta(1, 7).block(n, n, n, n) = (21.0*v5) + h1 * (-231.0*v6);
    tDelta(1, 8).resize(2*n, 2*n);
    tDelta(1, 8).block(0, 0, n, n) = 66.0*v6;
    tDelta(1, 8).block(0, n, n, n) = MatrixXs::Zero(n,n);
    tDelta(1, 8).block(n, 0, n, n) = 1.07142857143*v2_v3+1.0*v1_v4+2.5*v4_v1+1.5*v0_v5+-1.5*v5_v0+1.42857142857*v3_v2;
    tDelta(1, 8).block(n, n, n, n) = 66.0*v6;
    tH[1].resize(2*n, 2*n);
    tH[1].block(0, 0, n, n) = MatrixXs::Zero(n,n);     tH[1].block(0, n, n, n) = h1 * (MatrixXs::Identity(n, n));
    ...
    tH[1].block(n, n, n, n) = MatrixXs::Zero(n,n);     tDelta(2, 0) = MatrixXs::Zero(2*n, 2*n);
    tDelta(2, 1) = MatrixXs::Zero(2*n, 2*n);
    tDelta(2, 2) = MatrixXs::Zero(2*n, 2*n);
    tDelta(2, 3).resize(2*n, 2*n);
    tDelta(2, 3).block(0, 0, n, n) = (-0.5*v1) + h1 * (1.5*v2) + h2 * (-3.0*v3) + h3 * (5.0*v4) + h4 * (-7.5*v5) + h5 * (10.5*v6);
    tDelta(2, 3).block(0, n, n, n) = h1 * (-0.5*v1) + h2 * (0.5*v2) + h3 * (-0.5*v3) + h4 * (0.5*v4) + h5 * (-0.5*v5);
    tDelta(2, 3).block(n, 0, n, n) = (-1.5*v2) + h1 * (7.5*v3+-0.125*v0_v1+0.125*v1_v0) + h2 * (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+-22.5*v4) + h3 * (0.125*v3_v0+-0.125*v1_v2+-0.125*v0_v3+-0.125*v2_v1+52.5*v5) + h4 * (0.125*v2_v2+0.125*v3_v1+-105.0*v6+-0.125*v4_v0+0.125*v1_v3+0.125*v0_v4) + h5 * (-0.125*v2_v3+-0.125*v1_v4+-0.125*v4_v1+-0.125*v0_v5+0.125*v5_v0+-0.125*v3_v2);
    tDelta(2, 3).block(n, n, n, n) = (0.5*v1) + h1 * (-1.5*v2) + h2 * (3.0*v3) + h3 * (-5.0*v4) + h4 * (7.5*v5) + h5 * (-10.5*v6);
    tDelta(2, 4).resize(2*n, 2*n);
    tDelta(2, 4).block(0, 0, n, n) = (-1.5*v2) + h1 * (7.5*v3+-0.125*v0_v1+0.125*v1_v0) + h2 * (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+-22.5*v4) + h3 * (0.125*v3_v0+-0.125*v1_v2+-0.125*v0_v3+52.5*v5+-0.125*v2_v1) + h4 * (0.125*v2_v2+0.125*v1_v3+-105.0*v6+-0.125*v4_v0+0.125*v3_v1+0.125*v0_v4);
    tDelta(2, 4).block(0, n, n, n) = (0.5*v1) + h1 * (-1.5*v2) + h2 * (3.0*v3) + h3 * (-5.0*v4) + h4 * (7.5*v5);
    tDelta(2, 4).block(n, 0, n, n) = (-7.5*v3) + h1 * (-0.25*v1_v1+52.5*v4) + h2 * (0.25*v1_v2+-210.0*v5+0.75*v2_v1) + h3 * (-0.75*v2_v2+630.0*v6+-0.25*v1_v3+-1.5*v3_v1) + h4 * (0.75*v2_v3+2.5*v4_v1+0.25*v1_v4+1.5*v3_v2);
    tDelta(2, 4).block(n, n, n, n) = (1.5*v2) + h1 * (-0.125*v0_v1+-7.5*v3+0.125*v1_v0) + h2 * (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+22.5*v4) + h3 * (0.125*v3_v0+-0.125*v1_v2+-0.125*v0_v3+-52.5*v5+-0.125*v2_v1) + h4 * (0.125*v2_v2+0.125*v1_v3+105.0*v6+-0.125*v4_v0+0.125*v3_v1+0.125*v0_v4);
    tDelta(2, 5).resize(2*n, 2*n);
    tDelta(2, 5).block(0, 0, n, n) = (-5.0*v3+0.0833333333333*v0_v1+-0.0833333333333*v1_v0) + h1 * (-0.25*v0_v2+-0.25*v1_v1+0.25*v2_v0+35.0*v4) + h2 * (-0.5*v3_v0+0.416666666667*v1_v2+0.5*v0_v3+-140.0*v5+0.583333333333*v2_v1) + h3 * (-0.75*v2_v2+-0.666666666667*v1_v3+420.0*v6+0.833333333333*v4_v0+-1.08333333333*v3_v1+-0.833333333333*v0_v4);
    tDelta(2, 5).block(0, n, n, n) = (1.0*v2) + h1 * (-5.0*v3) + h2 * (15.0*v4) + h3 * (-35.0*v5);
    tDelta(2, 5).block(n, 0, n, n) = (0.0833333333333*v1_v1+-35.0*v4) + h1 * (-0.0208333333333*v0_v0_v1+0.0416666666667*v0_v1_v0+-1.0*v2_v1+-0.25*v1_v2+315.0*v5+-0.0208333333333*v1_v0_v0) + h2 * (0.0208333333333*v0_v1_v1+-0.0416666666667*v0_v2_v0+0.0208333333333*v1_v0_v1+1.5*v2_v2+0.5*v1_v3+-1575.0*v6+-0.0416666666667*v1_v1_v0+0.0208333333333*v0_v0_v2+4.25*v3_v1+0.0208333333333*v2_v0_v0) + h3 * (-2.25*v2_v3+-0.0208333333333*v1_v0_v2+-0.0208333333333*v2_v0_v1+-12.0833333333*v4_v1+-0.0208333333333*v0_v2_v1+-0.0208333333333*v1_v1_v1+0.0416666666667*v1_v2_v0+-0.0208333333333*v3_v0_v0+0.0416666666667*v0_v3_v0+0.0416666666667*v2_v1_v0+-5.25*v3_v2+-0.0208333333333*v0_v1_v2+-0.0208333333333*v0_v0_v3+-0.833333333333*v1_v4);
    tDelta(2, 5).block(n, n, n, n) = (0.0833333333333*v0_v1+5.0*v3+-0.0833333333333*v1_v0) + h1 * (-0.25*v0_v2+-0.25*v1_v1+0.25*v2_v0+-35.0*v4) + h2 * (-0.5*v3_v0+0.416666666667*v1_v2+0.5*v0_v3+140.0*v5+0.583333333333*v2_v1) + h3 * (-0.75*v2_v2+-0.666666666667*v1_v3+-420.0*v6+0.833333333333*v4_v0+-1.08333333333*v3_v1+-0.833333333333*v0_v4);
    tDelta(2, 6).resize(2*n, 2*n);
    tDelta(2, 6).block(0, 0, n, n) = (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+-17.5*v4) + h1 * (0.625*v3_v0+-0.5*v1_v2+-0.625*v0_v3+157.5*v5+-0.75*v2_v1) + h2 * (1.625*v2_v2+1.375*v1_v3+-787.5*v6+-1.875*v4_v0+2.625*v3_v1+1.875*v0_v4);
    tDelta(2, 6).block(0, n, n, n) = (2.5*v3) + h1 * (-17.5*v4) + h2 * (70.0*v5);
    tDelta(2, 6).block(n, 0, n, n) = (0.0104166666667*v0_v0_v1+-0.0208333333333*v0_v1_v0+0.375*v2_v1+0.125*v1_v2+-157.5*v5+0.0104166666667*v1_v0_v0) + h1 * (-0.03125*v0_v1_v1+0.0625*v0_v2_v0+-0.0416666666667*v1_v0_v1+-1.5*v2_v2+-0.625*v1_v3+1732.5*v6+0.0729166666667*v1_v1_v0+-0.03125*v0_v0_v2+-4.375*v3_v1+-0.03125*v2_v0_v0) + h2 * (4.125*v2_v3+-0.114583333333*v1_v2_v0+0.104166666667*v2_v0_v1+23.125*v4_v1+0.0729166666667*v0_v2_v1+0.0625*v1_v1_v1+0.0625*v1_v0_v2+0.0625*v3_v0_v0+-0.125*v0_v3_v0+-0.177083333333*v2_v1_v0+8.875*v3_v2+0.0520833333333*v0_v1_v2+0.0625*v0_v0_v3+1.875*v1_v4);
    tDelta(2, 6).block(n, n, n, n) = (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+17.5*v4) + h1 * (0.625*v3_v0+-0.5*v1_v2+-0.625*v0_v3+-157.5*v5+-0.75*v2_v1) + h2 * (1.625*v2_v2+1.375*v1_v3+787.5*v6+-1.875*v4_v0+2.625*v3_v1+1.875*v0_v4);
    tDelta(2, 7).resize(2*n, 2*n);
    tDelta(2, 7).block(0, 0, n, n) = (-0.25*v3_v0+0.2*v1_v2+0.25*v0_v3+-63.0*v5+0.3*v2_v1) + h1 * (-1.5*v2_v2+-1.25*v1_v3+693.0*v6+1.75*v4_v0+-2.5*v3_v1+-1.75*v0_v4);
    tDelta(2, 7).block(0, n, n, n) = (7.0*v4) + h1 * (-63.0*v5);
    tDelta(2, 7).block(n, 0, n, n) = (-0.0291666666667*v1_v1_v0+-0.025*v0_v2_v0+0.0166666666667*v1_v0_v1+0.6*v2_v2+0.25*v1_v3+-693.0*v6+0.0125*v0_v1_v1+0.0125*v0_v0_v2+1.5*v3_v1+0.0125*v2_v0_v0) + h1 * (-3.75*v2_v3+-0.0625*v1_v0_v2+-0.125*v2_v0_v1+-19.25*v4_v1+-0.075*v0_v2_v1+-0.0625*v1_v1_v1+0.1125*v1_v2_v0+-0.0625*v3_v0_v0+0.125*v0_v3_v0+0.2*v2_v1_v0+-7.5*v3_v2+-0.05*v0_v1_v2+-0.0625*v0_v0_v3+-1.75*v1_v4);
    tDelta(2, 7).block(n, n, n, n) = (-0.25*v3_v0+0.2*v1_v2+0.25*v0_v3+63.0*v5+0.3*v2_v1) + h1 * (-1.5*v2_v2+-1.25*v1_v3+-693.0*v6+1.75*v4_v0+-2.5*v3_v1+-1.75*v0_v4);
    tDelta(2, 8).resize(2*n, 2*n);
    tDelta(2, 8).block(0, 0, n, n) = 0.5*v2_v2+0.416666666667*v1_v3+-231.0*v6+-0.583333333333*v4_v0+0.833333333333*v3_v1+0.583333333333*v0_v4;
    tDelta(2, 8).block(0, n, n, n) = 21.0*v5;
    tDelta(2, 8).block(n, 0, n, n) = 1.25*v2_v3+0.0208333333333*v1_v0_v2+0.0416666666667*v2_v0_v1+5.83333333333*v4_v1+0.025*v0_v2_v1+0.0208333333333*v1_v1_v1+-0.0375*v1_v2_v0+0.0208333333333*v3_v0_v0+-0.0416666666667*v0_v3_v0+-0.0666666666667*v2_v1_v0+2.5*v3_v2+0.0166666666667*v0_v1_v2+0.0208333333333*v0_v0_v3+0.583333333333*v1_v4;
    tDelta(2, 8).block(n, n, n, n) = 0.5*v2_v2+0.416666666667*v1_v3+231.0*v6+-0.583333333333*v4_v0+0.833333333333*v3_v1+0.583333333333*v0_v4;
    tH[2].resize(2*n, 2*n);
    ...
    tH[2].block(0, n, n, n) = MatrixXs::Zero(n,n);     tH[2].block(n, 0, n, n) = h3 * (-1.5*v2);
    ...
    tDelta(3, 5).block(0, 0, n, n) = (-0.166666666667*v0_v1+2.5*v3+0.166666666667*v1_v0) + h1 * (0.5*v0_v2+0.25*v1_v1+-0.5*v2_v0+-17.5*v4) + h2 * (1.0*v3_v0+-0.583333333333*v1_v2+-1.0*v0_v3+70.0*v5+-0.416666666667*v2_v1) + h3 * (0.75*v2_v2+1.08333333333*v1_v3+-210.0*v6+-1.66666666667*v4_v0+0.666666666667*v3_v1+1.66666666667*v0_v4);
    tDelta(3, 5).block(0, n, n, n) = (-0.5*v2) + h1 * (2.5*v3+-0.125*v0_v1+0.125*v1_v0) + h2 * (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+-7.5*v4) + h3 * (0.125*v3_v0+-0.125*v1_v2+-0.125*v0_v3+-0.125*v2_v1+17.5*v5);
    tDelta(3, 5).block(n, 0, n, n) = (-0.375*v0_v2+-0.291666666667*v1_v1+0.375*v2_v0+17.5*v4) + h1 * (-0.0208333333333*v1_v0_v0+0.0416666666667*v0_v1_v0+-1.875*v3_v0+1.875*v0_v3+1.25*v2_v1+1.25*v1_v2+-157.5*v5+-0.0208333333333*v0_v0_v1) + h2 * (0.0208333333333*v0_v1_v1+-0.0416666666667*v0_v2_v0+0.0208333333333*v1_v0_v1+-3.375*v2_v2+-3.625*v1_v3+787.5*v6+-0.0416666666667*v1_v1_v0+5.625*v4_v0+0.0208333333333*v0_v0_v2+-3.625*v3_v1+0.0208333333333*v2_v0_v0+-5.625*v0_v4) + h3 * (7.5*v2_v3+-0.0208333333333*v1_v0_v2+-0.0208333333333*v2_v0_v1+8.54166666667*v4_v1+-0.0208333333333*v0_v2_v1+-0.0208333333333*v1_v1_v1+0.0416666666667*v1_v2_v0+-0.0208333333333*v3_v0_v0+0.0416666666667*v0_v3_v0+13.125*v0_v5+-13.125*v5_v0+0.0416666666667*v2_v1_v0+7.5*v3_v2+-0.0208333333333*v0_v1_v2+-0.0208333333333*v0_v0_v3+8.54166666667*v1_v4);
    tDelta(3, 5).block(n, n, n, n) = (-2.5*v3+0.0833333333333*v0_v1+-0.0833333333333*v1_v0) + h1 * (-0.25*v0_v2+-0.25*v1_v1+0.25*v2_v0+17.5*v4) + h2 * (-0.5*v3_v0+0.416666666667*v1_v2+0.5*v0_v3+-70.0*v5+0.583333333333*v2_v1) + h3 * (-0.75*v2_v2+-0.666666666667*v1_v3+210.0*v6+0.833333333333*v4_v0+-1.08333333333*v3_v1+-0.833333333333*v0_v4);
    tDelta(3, 6).resize(2*n, 2*n);
    tDelta(3, 6).block(0, 0, n, n) = (-0.375*v0_v2+-0.291666666667*v1_v1+0.375*v2_v0+17.5*v4) + h1 * (-0.0208333333333*v1_v0_v0+0.0416666666667*v0_v1_v0+-1.875*v3_v0+1.875*v0_v3+1.25*v2_v1+1.25*v1_v2+-157.5*v5+-0.0208333333333*v0_v0_v1) + h2 * (-0.0416666666667*v1_v1_v0+-0.0416666666667*v0_v2_v0+0.0208333333333*v1_v0_v1+-3.375*v2_v2+-3.625*v1_v3+787.5*v6+0.0208333333333*v0_v1_v1+5.625*v4_v0+0.0208333333333*v0_v0_v2+-3.625*v3_v1+0.0208333333333*v2_v0_v0+-5.625*v0_v4);
    tDelta(3, 6).block(0, n, n, n) = (-2.5*v3+0.0833333333333*v0_v1+-0.0833333333333*v1_v0) + h1 * (-0.25*v0_v2+-0.25*v1_v1+0.25*v2_v0+17.5*v4) + h2 * (-0.5*v3_v0+0.416666666667*v1_v2+0.5*v0_v3+0.583333333333*v2_v1+-70.0*v5);
    tDelta(3, 6).block(n, 0, n, n) = (-0.0104166666667*v1_v0_v0+0.0208333333333*v0_v1_v0+1.25*v3_v0+-1.25*v0_v3+-1.375*v2_v1+-1.125*v1_v2+157.5*v5+-0.0104166666667*v0_v0_v1) + h1 * (0.0520833333333*v1_v1_v0+-0.0625*v0_v2_v0+-0.0416666666667*v1_v0_v1+7.5*v2_v2+6.875*v1_v3+-1732.5*v6+-0.0104166666667*v0_v1_v1+-8.75*v4_v0+0.03125*v0_v0_v2+9.375*v3_v1+0.03125*v2_v0_v0+8.75*v0_v4) + h2 * (-26.375*v2_v3+-0.0104166666667*v1_v2_v0+0.145833333333*v2_v0_v1+-38.125*v4_v1+0.0520833333333*v0_v2_v1+0.0625*v1_v1_v1+0.0208333333333*v1_v0_v2+-0.0625*v3_v0_v0+0.125*v0_v3_v0+-35.0*v0_v5+35.0*v5_v0+-0.197916666667*v2_v1_v0+-29.875*v3_v2+-0.0104166666667*v0_v1_v2+-0.0625*v0_v0_v3+-25.625*v1_v4);
    tDelta(3, 6).block(n, n, n, n) = (0.125*v0_v2+0.208333333333*v1_v1+-0.125*v2_v0+-17.5*v4) + h1 * (-0.0208333333333*v1_v0_v0+0.0416666666667*v0_v1_v0+0.625*v3_v0+-0.625*v0_v3+-1.25*v2_v1+-0.75*v1_v2+157.5*v5+-0.0208333333333*v0_v0_v1) + h2 * (0.0208333333333*v0_v1_v1+-0.0416666666667*v0_v2_v0+0.0208333333333*v1_v0_v1+2.625*v2_v2+1.875*v1_v3+-787.5*v6+-0.0416666666667*v1_v1_v0+-1.875*v4_v0+0.0208333333333*v0_v0_v2+4.375*v3_v1+0.0208333333333*v2_v0_v0+1.875*v0_v4);
    tDelta(3, 7).resize(2*n, 2*n);
    tDelta(3, 7).block(0, 0, n, n) = (0.0104166666667*v1_v0_v0+-0.0208333333333*v0_v1_v0+1.0*v3_v0+-1.0*v0_v3+-0.825*v2_v1+-0.675*v1_v2+94.5*v5+0.0104166666667*v0_v0_v1) + h1 * (0.0729166666667*v1_v1_v0+0.0625*v0_v2_v0+-0.0416666666667*v1_v0_v1+4.5*v2_v2+4.375*v1_v3+-1039.5*v6+-0.03125*v0_v1_v1+-7.0*v4_v0+-0.03125*v0_v0_v2+5.625*v3_v1+-0.03125*v2_v0_v0+7.0*v0_v4);
    tDelta(3, 7).block(0, n, n, n) = (0.125*v0_v2+0.125*v1_v1+-0.125*v2_v0+-10.5*v4) + h1 * (0.625*v3_v0+-0.5*v1_v2+-0.625*v0_v3+-0.75*v2_v1+94.5*v5);
    tDelta(3, 7).block(n, 0, n, n) = (-0.00833333333333*v0_v1_v1+0.0375*v0_v2_v0+-0.00416666666667*v1_v0_v1+-4.275*v2_v2+-3.5*v1_v3+1039.5*v6+0.0125*v1_v1_v0+4.375*v4_v0+-0.01875*v0_v0_v2+-6.0*v3_v1+-0.01875*v2_v0_v0+-4.375*v0_v4) + h1 * (31.875*v2_v3+-0.0078125*v0_v1_v0_v0+0.03125*v1_v0_v2+-0.15625*v2_v0_v1+55.125*v4_v1+-0.0125*v0_v2_v1+-0.0416666666667*v1_v1_v1+-0.075*v1_v2_v0+0.09375*v0_v0_v3+-0.1875*v0_v3_v0+39.375*v0_v5+-39.375*v5_v0+0.16875*v2_v1_v0+39.375*v3_v2+0.04375*v0_v1_v2+0.0078125*v0_v0_v1_v0+0.09375*v3_v0_v0+0.00260416666667*v1_v0_v0_v0+-0.00260416666667*v0_v0_v0_v1+28.875*v1_v4);
    tDelta(3, 7).block(n, n, n, n) = (0.0104166666667*v1_v0_v0+-0.0208333333333*v0_v1_v0+-0.25*v3_v0+0.25*v0_v3+0.675*v2_v1+0.325*v1_v2+-94.5*v5+0.0104166666667*v0_v0_v1) + h1 * (-0.03125*v0_v1_v1+0.0625*v0_v2_v0+-0.0416666666667*v1_v0_v1+-3.0*v2_v2+-1.875*v1_v3+1039.5*v6+0.0729166666667*v1_v1_v0+1.75*v4_v0+-0.03125*v0_v0_v2+-5.625*v3_v1+-0.03125*v2_v0_v0+-1.75*v0_v4);
    tDelta(3, 8).resize(2*n, 2*n);
    tDelta(3, 8).block(0, 0, n, n) = 0.0125*v0_v1_v1+-0.025*v0_v2_v0+0.0166666666667*v1_v0_v1+-1.9*v2_v2+-1.83333333333*v1_v3+462.0*v6+-0.0291666666667*v1_v1_v0+2.91666666667*v4_v0+0.0125*v0_v0_v2+-2.66666666667*v3_v1+0.0125*v2_v0_v0+-2.91666666667*v0_v4;
    tDelta(3, 8).block(0, n, n, n) = -0.25*v3_v0+0.2*v1_v2+0.25*v0_v3+0.3*v2_v1+-42.0*v5;
    tDelta(3, 8).block(n, 0, n, n) = -13.0*v2_v3+0.003125*v0_v1_v0_v0+-0.0166666666667*v1_v0_v2+0.0166666666667*v2_v0_v1+-25.6666666667*v4_v1+-0.0125*v0_v2_v1+0.0375*v1_v2_v0+-0.0416666666667*v3_v0_v0+0.0833333333333*v0_v3_v0+-15.75*v0_v5+15.75*v5_v0+-0.00416666666667*v2_v1_v0+-17.0*v3_v2+-0.0208333333333*v0_v1_v2+-0.003125*v0_v0_v1_v0+-0.0416666666667*v0_v0_v3+-0.00104166666667*v1_v0_v0_v0+0.00104166666667*v0_v0_v0_v1+-11.6666666667*v1_v4;
    tDelta(3, 8).block(n, n, n, n) = 0.0125*v0_v1_v1+-0.025*v0_v2_v0+0.0166666666667*v1_v0_v1+1.1*v2_v2+0.666666666667*v1_v3+-462.0*v6+-0.0291666666667*v1_v1_v0+-0.583333333333*v4_v0+0.0125*v0_v0_v2+2.33333333333*v3_v1+0.0125*v2_v0_v0+0.583333333333*v0_v4;
    tH[3].resize(2*n, 2*n);
    ...
    tDelta(4, 7).block(0, 0, n, n) = (-0.03125*v1_v0_v0+0.0625*v0_v1_v0+-0.75*v3_v0+0.75*v0_v3+0.275*v2_v1+0.225*v1_v2+-31.5*v5+-0.03125*v0_v0_v1) + h1 * (0.0520833333333*v0_v1_v1+-0.1875*v0_v2_v0+0.0416666666667*v1_v0_v1+-1.5*v2_v2+-1.875*v1_v3+346.5*v6+-0.09375*v1_v1_v0+5.25*v4_v0+0.09375*v0_v0_v2+-1.875*v3_v1+0.09375*v2_v0_v0+-5.25*v0_v4);
    tDelta(4, 7).block(0, n, n, n) = (-0.125*v0_v2+-0.0416666666667*v1_v1+0.125*v2_v0+3.5*v4) + h1 * (-0.0208333333333*v1_v0_v0+0.0416666666667*v0_v1_v0+-0.625*v3_v0+0.625*v0_v3+-31.5*v5+0.25*v1_v2+0.25*v2_v1+-0.0208333333333*v0_v0_v1);
    tDelta(4, 7).block(n, 0, n, n) = (-0.0458333333333*v0_v1_v1+0.1125*v0_v2_v0+-0.0541666666667*v1_v0_v1+1.425*v2_v2+2.0*v1_v3+-346.5*v6+0.1*v1_v1_v0+-4.375*v4_v0+-0.05625*v0_v0_v2+2.0*v3_v1+-0.05625*v2_v0_v0+4.375*v0_v4) + h1 * (-13.125*v2_v3+-0.0078125*v0_v1_v0_v0+0.21875*v1_v0_v2+0.21875*v2_v0_v1+-18.375*v4_v1+0.2125*v0_v2_v1+0.145833333333*v1_v1_v1+-0.4125*v1_v2_v0+0.28125*v0_v0_v3+-0.5625*v0_v3_v0+-39.375*v0_v5+39.375*v5_v0+-0.43125*v2_v1_v0+-13.125*v3_v2+0.19375*v0_v1_v2+0.0078125*v0_v0_v1_v0+0.28125*v3_v0_v0+0.00260416666667*v1_v0_v0_v0+-0.00260416666667*v0_v0_v0_v1+-18.375*v1_v4);
    tDelta(4, 7).block(n, n, n, n) = (0.0104166666667*v1_v0_v0+-0.0208333333333*v0_v1_v0+0.5*v3_v0+-0.5*v0_v3+-0.225*v2_v1+-0.275*v1_v2+31.5*v5+0.0104166666667*v0_v0_v1) + h1 * (-0.03125*v0_v1_v1+0.0625*v0_v2_v0+-0.0416666666667*v1_v0_v1+1.5*v2_v2+1.875*v1_v3+-346.5*v6+0.0729166666667*v1_v1_v0+-3.5*v4_v0+-0.03125*v0_v0_v2+1.875*v3_v1+-0.03125*v2_v0_v0+3.5*v0_v4);
    tDelta(4, 8).resize(2*n, 2*n);
    tDelta(4, 8).block(0, 0, n, n) = 0.1*v1_v1_v0+0.1125*v0_v2_v0+-0.0541666666667*v1_v0_v1+1.425*v2_v2+2.0*v1_v3+-346.5*v6+-0.0458333333333*v0_v1_v1+-4.375*v4_v0+-0.05625*v0_v0_v2+2.0*v3_v1+-0.05625*v2_v0_v0+4.375*v0_v4;
    tDelta(4, 8).block(0, n, n, n) = 0.0104166666667*v1_v0_v0+-0.0208333333333*v0_v1_v0+0.5*v3_v0+-0.5*v0_v3+31.5*v5+-0.275*v1_v2+-0.225*v2_v1+0.0104166666667*v0_v0_v1;
    tDelta(4, 8).block(n, 0, n, n) = 13.5*v2_v3+-0.00625*v0_v1_v0_v0+-0.175*v1_v0_v2+-0.2625*v2_v0_v1+19.25*v4_v1+-0.1625*v0_v2_v1+-0.145833333333*v1_v1_v1+0.3*v1_v2_v0+-0.125*v0_v0_v3+0.25*v0_v3_v0+31.5*v0_v5+-31.5*v5_v0+0.425*v2_v1_v0+12.75*v3_v2+-0.125*v0_v1_v2+0.00625*v0_v0_v1_v0+-0.125*v3_v0_v0+0.00208333333333*v1_v0_v0_v0+-0.00208333333333*v0_v0_v0_v1+17.5*v1_v4;
    tDelta(4, 8).block(n, n, n, n) = -0.0458333333333*v1_v1_v0+-0.0125*v0_v2_v0+0.0291666666667*v1_v0_v1+-1.575*v2_v2+-1.75*v1_v3+346.5*v6+0.0166666666667*v0_v1_v1+2.625*v4_v0+0.00625*v0_v0_v2+-1.75*v3_v1+0.00625*v2_v0_v0+-2.625*v0_v4;
    tH[4].resize(2*n, 2*n);
