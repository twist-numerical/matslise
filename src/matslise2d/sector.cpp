...
Matslise2D<Scalar>::Sector::Sector(const Matslise2D<Scalar> *se2d, const Scalar &ymin, const Scalar &ymax,
    ...
    if (se2d->options.nestedOptions._symmetric) {
        ...
        quadrature = std::make_unique<BasisQuadratureHalf<Scalar, 16>>(
        ...
        quadrature = std::make_unique<BasisQuadrature<Scalar, 16>>(
    ...
    func_eigenfunctions.resize(se2d->N);
    Scalar E;
    int index;
    ...
    matscs = new typename Matscs<Scalar>::Sector(
            legendre::getCoefficients<MATSCS_N, MatrixXs, Scalar>([&, this](Scalar y) -> MatrixXs {
                /*
                cout << "-----" << endl;
                cout << "\n" << quadrature->dV(y) << endl << endl;
                cout << "\n" << this->calculateDeltaV(y) << endl << endl;
                cout << "\n" << (this->calculateDeltaV(y) - quadrature->dV(y)) << endl << endl;
                */
                return quadrature->dV(y);
