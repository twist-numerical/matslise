...
class ParsedInput extends Component {
    ...
    return (
      <div
        ...
        dangerouslySetInnerHTML={{
          __html:
            "<span>" +
            htmlEscape(this.state.done) +
            '</span><span style="color: red;">' +
            htmlEscape(this.state.todo) +
            "</span>"
        }}
        onKeyDown={e => {
          if (e.keyCode === 13) {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.cursorPosition >= 0) {
      if (this.cursorPosition <= this.state.done.length)
        ...
        this.setCursor(
          this.input.children[1].firstChild,
          this.cursorPosition - this.state.done.length
        );
    }
  ...
  onInput(value) {
    ...
    if (parsed !== null) this.props.onParsed(parsed);
  ...
  parse(value) {
    ...
    try {
      ...
      if (e.done === undefined || e.todo === undefined) throw e;
      done = e.done;
      todo = e.todo;
    }
    return {
