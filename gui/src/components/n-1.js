...
class Nm1 extends Component {
  ...
  normalize(f) {
    ...
    const scale = Math.max(
      ...Array.from({ length: n }, (x, i) => {
        return f(xmin + ((i + 0.2 + 0.6 * Math.random()) / n) * (xmax - xmin));
      }).map(Math.abs)
    );
  ...
  render() {
    if (!this.state.matsliseLoaded) return "Loading";
    while (this.toDelete.length) this.toDelete.pop().delete();
    ...
    const eigenfunctions = eigenvalues.map(e => {
      const f = matslise.eigenfunction(e.second, [0, 1], [0, 1]);
      this.toDelete.push(f);
      const normalized = this.normalize(x => f(x)[0]);
      return x => normalized(x) + e.second;
    });
            ...
            return (
              ...
              >
                <Label
                  ...
                  y={value}
                  content={({ viewBox: { x, y } }) => {
                    return index === eigenvalues.length - 1 ? (
                      <text
                        x={x + 30}
                        y={y + 100}
                        fill="#666666"
                      ...
                      <text x={x} y={y + 65} fill="#666666" fontSize="35px">
                        b<tspan baselineShift="sub">{index}</tspan>
                        <tspan dx="-15px" baselineShift="super">
                          (k)
                        </tspan>
