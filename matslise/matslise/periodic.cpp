...
Scalar withinInterval(const PeriodicMatslise<Scalar> *ms, Scalar eMin, Scalar eMax) {
    ...
    while (eMax - eMin > 1e-6) {
        Scalar e = segment ? eMin + (eMax - eMin) / (1 + fMax / fMin) : (eMin + eMax) / 2;
        segment = !segment;
...
template<typename Scalar>
vector<tuple<int, Scalar, vector<unique_ptr<typename PeriodicMatslise<Scalar>::Eigenfunction>>>>
eigenpairsHelper(const PeriodicMatslise<Scalar> *ms, Scalar eMin, Scalar eMax, int iMin, int iMax) {
    ...
    vector<tuple<int, Scalar, vector<unique_ptr<typename PeriodicMatslise<Scalar>::Eigenfunction>>>> result;
    ...
    for (int i = iMin; i < iMax; ++i) {
        ...
        if (i % 2 == 1 && nextLow - e < 1e-8) {
            // double eigenvalue
            vector<unique_ptr<typename PeriodicMatslise<Scalar>::Eigenfunction>> eigenfunctions;
            eigenfunctions.reserve(2);
            eigenfunctions.emplace_back(ms->matslise.eigenfunction(e, boundary.col(0), boundary.col(0)));
            eigenfunctions.emplace_back(std::move(ms->matslise.eigenfunction(e, boundary.col(1), boundary.col(1))));
            result.emplace_back(i, e, std::move(eigenfunctions));
            ++i;
        } else {
...
PeriodicMatslise<Scalar>::eigenpairsByIndex(int iMin, int iMax) const {
    ...
    while (matchingError(eMin).second.minCoeff() > iMin)
        eMin *= 2;

    while (matchingError(eMax).second.maxCoeff() < iMax + 1)
        eMax *= 2;
    return eigenpairsHelper(this, eMin, eMax, iMin, iMax);
}
